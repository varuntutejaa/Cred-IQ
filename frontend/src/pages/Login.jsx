...
export default function Login() {
  ...
  const { login, demoLogin, demoRecruiterLogin } = useAuth()
            ...
            <motion.div key={role} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -6 }} transition={{ duration: 0.15 }}>
              ...
              <div className="space-y-2 mb-4">
                <button onClick={handleDemo}
                  className="w-full flex items-center justify-center gap-2 glass glass-hover border border-white/15 rounded-xl py-3 text-sm font-medium"
                >
                  ...
                  Continue with GitHub {role === 'recruiter' ? '(Recruiter)' : ''}
                </button>
              ...
              {role === 'developer' && (
                <div className="glass rounded-xl p-3 text-xs text-dark-300 leading-relaxed mb-4 border border-brand-500/15 bg-brand-500/5">
                  <span className="text-brand-300 font-semibold">Developer flow: </span>
                  GitHub Login → Auto-import repos → Generate verified profile
                </div>
              )}
              {role === 'recruiter' && (
                <div className="glass rounded-xl p-3 text-xs text-dark-300 leading-relaxed mb-4 border border-purple-500/15 bg-purple-500/5">
                  <span className="text-purple-300 font-semibold">Recruiter flow: </span>
                  Sign in → Search candidates → Verify & compare profiles
                </div>
              )}
            </motion.div>
          </AnimatePresence>
